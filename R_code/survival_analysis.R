...
##############################################################################################################################
NUMBER.NA <- 999999.999 # using fill NA

##############################################################################################################################
# binom.confint: confidence interval on the binomial probability
...
plot.ROC <- function(Y, marker=NULL, outcome=NULL, cutoff=NULL, jpg.width=1000*sqrt(2), jpg.pointsize=30, lwd=4, jpg.file=NULL) {
  ...
  out.file <- jpg.file 
  if (!is.null(cutoff)) out.file <- paste(out.file, "_cutoff", signif(cutoff, 4), sep="")
  out.file <- paste(out.file,"AUC=", auc, "ROC.jpg", sep="_")
  ...
  if (length(index.optimal) == 1) {
    ...
    ###### add
    result <- c(auc, as.numeric(round(sen[index.optimal], 1)), as.numeric(round(spe[index.optimal], 1)), out.file)
    ######
  }
  else {
    legend("bottomright", legend=legend.auc, inset=0.02, cex=0.9, lwd=lwd)
    ###### add
    result <- c(auc, NA, NA, out.file)
    ######
  }
  if (!is.null(jpg.file)) dev.off()
  # return(out.file)
...
plot.kaplanmeier <- function(marker, time, event, cutoff=NULL, surv.test="sctest", conf.level=95, colors=c("black", "red"), jpg.width=1000*sqrt(2), jpg.height=1000, jpg.pointsize=30, lwd=3, jpg.file=NULL) {
  ...
  if (!is.null(cutoff)) {
    ...
    out.file <- paste(jpg.file, "_cutoff=", signif(cutoff, 4), sep="")
    #add
    out.file <- paste(out.file, "_p=", p, sep="")
    out.file <- paste(out.file, "_HR=",HR, sep="")
    #add
    ...
    legend("topright", legend=paste(method,": ",mk,", ","HR = ", round(HR, 2), " (", round(HR.lower, 2), "-", round(HR.upper, 2), "), p = ", signif(p, 2), sep=""), bty="n")
    if (!is.null(jpg.file)) dev.off()
  }
  else out.file <- NULL
  ##### add
  result <- c(p[1][[1]], HR, out.file)
  #####
  # return(out.file)
  ##### add
  return (result)
...
get.cutoff <- function(type=c("none", "distribution", "outcome_significance", "outcome_euclidean", "outcome_manhattan", "outcome_sensitivity", "outcome_specificity", "survival_significance", "manual"), filename, biomarker=NULL, outcome=NULL, time=NULL, event=NULL, cutoff=NULL, threshold=NULL, plots=c("histogram", "OR", "ROC", "HR", " time", "waterfall", "kaplanmeier"), nmin=10) {
  ...
  if (nbiomarker < 2*nmin) {
    msg[length(msg)+1] <- paste("<br>ERROR: Number of patients must be greater than ", 2*nmin, ".", sep="")
    ###### add
    result[1] <- "True" # ERROR
    ######
  }
    ...
    if (type.variable == "outcome" || "OR" %in% plots || "ROC" %in% plots) {
      ...
      if (length(ind) < 2*nmin) {
        msg[length(msg)+1] <- paste("<br>ERROR: Number of patients with outcome information must be greater than ", 2*nmin, ".", sep="")
        ###### add
        result[1] <- "True" # ERROR
        ######
      }
    ...
    if (type.variable == "survival" || "HR" %in% plots || "time" %in% plots) {
      ...
      if (length(ind) < 2*nmin) {
        msg[length(msg)+1] <- paste("<br>ERROR: Number of patients with survival information must be greater than ", 2*nmin, ".", sep="")
        ###### add
        result[1] <- "True" # ERROR
        ######
      }
    ...
    if (!is.null(cutoff)) {
      msg[length(msg)+1] <- paste("<br>Optimal cutoff value using method &quot", type, "&quot: ", signif(cutoff, 4), sep="")
      ###### add
      result[2] <- as.numeric(cutoff) # cutoff
      ######
    }
    else {
      ###### add
      ...
      ######
      if ("type" != "none") {
        msg[length(msg)+1] <- paste("<br>WARNING: Could not determine Optimal cutoff value using method &quot", type, "&quot!", sep="")
        ###### add
        result[1] <- "True" # ERROR
        ######
      }
    ...
    if ("histogram" %in% plots) {
      ...
      ###### add
      if (!is.null(status) && status == "try-error") result[1] <- "True" # ERROR
      else if (strsplit(res, split=" ")[[1]][1]=="Error") result[1] <- "True" # ERROR
      else {
        if (is.na(result[8])) result[8] <- res
        else result[8] <- paste(result[8], ', ', res, sep="") # graph name
      }
      ######
    }
    if ("OR" %in% plots) {
    ...
    if ("ROC" %in% plots) {
      ...
      ###### add
      if (!is.null(status) && status == "try-error") result[1] <- "True" # ERROR
      else if (strsplit(res[4], split=" ")[[1]][1]=="Error") result[1] <- "True" # ERROR
      else {
        result[5] <- as.numeric(res[1]) # AUC
        result[6] <- as.numeric(res[2]) # sen
        result[7] <- as.numeric(res[3]) # spe
        if (is.na(result[8])) result[8] <- res[4]
        else result[8] <- paste(result[8], ', ', res[4], sep="") # graph name
      }
      ######
    }
    ...
    if (!is.null(cutoff)) {
      ...
      if ("kaplanmeier" %in% plots && length(ind) >= 2*nmin) {
        ...
        ###### add
        if (!is.null(status) && status == "try-error") result[1] <- "True" # ERROR
        else if (strsplit(res[3], split=" ")[[1]][1]=="Error") result[1] <- "True" # ERROR
        else {
          result[3] <- as.numeric(res[1]) # p-value
          result[4] <- as.numeric(res[2]) # HR
          if (is.na(result[8])) result[8] <- res[3]
          else result[8] <- paste(result[8], ', ', res[3], sep="") # graph name
        }
        ######
      }
    }
...
file_name <- "TCGA_Uterus_survival"
cate <- strsplit(file_name, split="_")[[1]][2] # category

# make save folder
folder_path <- paste(base_path, "/", file_name, sep="")
dir.create(folder_path)

# load data
file_path <- paste(file_name,".csv", sep="")

input <- read.table(file_path, sep = ",", header = TRUE)
Markers <- colnames(input)[4:442]
list.methods <- c("distribution","outcome_significance")


# category / SLC / method / error(T/F) / cutoff / p-value / HR / AUC / sensitivity / specificity / graph_name_jpg (,)
df <- data.frame(
  category = character(),
  SLC = character(),
  method = character(),
...
for(method in list.methods){
  colr <- list()
  c = 0
  for(mk in Markers){ # 439
    c <- c + 1
    
    coli <- match(mk, colnames(input))
    res  <- get.cutoff(
      type = method,
      filename = paste(file_name,"/",mk, sep=""), # add savepath
      biomarker = input[, coli],
      outcome = input$OS,
      time = input$OS.time,
      event = input$OS,
      plots = c("histogram", "ROC", "kaplanmeier"),
      nmin = 10
    )
...
# save file
output_file <- paste(file_name,"_","summary.csv", sep="")
